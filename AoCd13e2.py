...
def parse_instructions(raw_instructions):
    ...
    for instruction in raw_instructions.split("\n\n"):
        ...
        instructions.append({"A": (int(ax[2:]), int(ay[2:])), "B": (int(bx[2:]), int(by[2:])), "P": (int(px[2:]), int(py[2:]))})
    return instructions

def correct_instructions(instructions):
    for instruction in instructions:
        px, py = instruction["P"]
        instruction["P"] = (px + 10000000000000, py + 10000000000000)
    return instructions

def get_solution(instruction):
    ax, ay = instruction["A"]
    bx, by = instruction["B"]
    px, py = instruction["P"]
    a, b = sympy.symbols("a b")
    x_eq = sympy.Eq(ax * a + bx * b, px)
    y_eq = sympy.Eq(ay * a + by * b, py)
    solution = sympy.solve([x_eq, y_eq], [a, b])
    if type(solution[a]) == sympy.core.numbers.Rational or type(solution[b]) == sympy.core.numbers.Rational:
        return None
    return solution
...
def get_solutions(instructions):
    return [get_solution(instruction) for instruction in instructions]

def get_costs(solutions):
    return [get_cost(solution) for solution in solutions]

instructions = parse_instructions(full_instructions)
print(instructions)

instructions = correct_instructions(instructions)
print(instructions)

solutions = get_solutions(instructions)
print(solutions)
